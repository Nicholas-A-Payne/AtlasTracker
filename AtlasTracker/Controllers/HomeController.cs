...
namespace AtlasTracker.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<JsonResult> GglProjectPriority()
        {
            ...
            foreach (string priority in Enum.GetNames(typeof(EnumProjectPriority)))
            {
        ...
        public async Task<JsonResult> PlotlyBarChart()
        {
            ...
            {
                X = projects.Select(p => p.Name).ToArray(),
                Y = projects.Select(async p => (await _projectService.GetProjectMembersByRoleAsync(p.Id, nameof(AppRole.Developer))).Count).Select(c => c.Result).ToArray(),
                Name = "Developers",
                Type = "bar"
            };
        ...
        public async Task<JsonResult> AmCharts()
        {
            ...
            foreach (Project project in projects)
            {
                ...
                item.Developers = (await _projectService.GetProjectMembersByRoleAsync(project.Id, nameof(AppRole.Developer))).Count();
